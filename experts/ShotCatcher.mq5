...
input string InpTimeFilter = "01:20-09:00, 09:02-23:45"; // Час роботи (HH:MM-HH:MM,...)
...
bool IsInTimeFilter()
{
   ...
   string ranges[];
   int cnt = StringSplit(InpTimeFilter, ',', ranges);
   for(int r = 0; r < cnt; r++)
   {
      ...
      string parts[];
      if(StringSplit(range, '-', parts) != 2) continue;
      string fromStr = parts[0]; StringTrimLeft(fromStr); StringTrimRight(fromStr);
      string toStr   = parts[1]; StringTrimLeft(toStr);   StringTrimRight(toStr);
      string fromParts[], toParts[];
      if(StringSplit(fromStr, ':', fromParts) != 2) continue;
      if(StringSplit(toStr,   ':', toParts)   != 2) continue;
      int fromMin = (int)StringToInteger(fromParts[0]) * 60 + (int)StringToInteger(fromParts[1]);
      int toMin   = (int)StringToInteger(toParts[0])   * 60 + (int)StringToInteger(toParts[1]);
      if(fromMin <= toMin)
      {
...
void OnTimer()
{
   if(!IsInTimeFilter())
   {
      if(g_buyTicket > 0 && OrderSelect(g_buyTicket))
         { trade.OrderDelete(g_buyTicket); g_buyTicket = 0; }
      if(g_sellTicket > 0 && OrderSelect(g_sellTicket))
         { trade.OrderDelete(g_sellTicket); g_sellTicket = 0; }
      return;
   }
